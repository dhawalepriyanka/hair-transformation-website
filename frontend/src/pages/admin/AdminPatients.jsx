...
const statuses = ['All', 'In Consultation', 'Completed', 'Cancelled'];
...
export default function AdminPatients() {
  ...
  const [visits, setVisits] = useState([]); const [query, setQuery] = useState(''); const [status, setStatus] = useState('All'); const [date, setDate] = useState(''); const [error, setError] = useState(''); const [loading, setLoading] = useState(true);
  ...
  useEffect(() => { Promise.all([fetchPatientVisits(),fetchProducts()]).then(([patientData,productData])=>{setVisits(patientData);setProducts(productData);if(patientData.length)setActiveVisit(patientData[0]);}).catch(e => setError(e.message)).finally(() => setLoading(false)); }, []);
  useEffect(() => { setShowRegistration(location.pathname === '/admin/add-patient'); }, [location.pathname]);
  const filtered = useMemo(() => visits.filter(v => `${v.patient_name} ${v.mobile} ${v.concern}`.toLowerCase().includes(query.toLowerCase()) && (status === 'All' || v.status === status) && (!date || String(v.visit_at).slice(0, 10) === date)), [visits, query, status, date]);
  const todayCount = visits.filter(v => new Date(v.visit_at).toDateString() === new Date().toDateString()).length;
  ...
  return <section className="admin-patients-page"><div className="container">
    <header className="admin-patient-header"><div><span className="eyebrow">CLINIC WORKSPACE</span><h1 className="serif">Patients Sent to Clinic</h1><p>Patients handed over by reception or registered by Admin.</p></div><div className="admin-patient-header-actions"><span className="live-indicator"><i/> Live patient queue</span><button type="button" className="primary-button" aria-expanded={showRegistration} aria-controls="admin-patient-registration" onClick={() => setShowRegistration(value => !value)}><Plus size={18}/> Add Patient</button></div></header>

    {showRegistration && <form id="admin-patient-registration" className="clinic-panel clinic-form admin-registration-form" onSubmit={registerPatient}><h2 className="serif">Add Patient</h2><PatientRegistrationFields form={form} onChange={e => setForm(current => ({ ...current, [e.target.name]: e.target.value }))}/>{registrationError && <div className="form-alert" role="alert">{registrationError}</div>}{registrationMessage && <div className="form-success" role="status">{registrationMessage}</div>}<button className="primary-button" disabled={registering}>{registering ? 'Saving…' : 'Register Patient'}</button></form>}

    <div className="patient-stat-grid"><article><span className="stat-icon"><UsersRound /></span><div><strong>{visits.length}</strong><small>Patients received</small></div></article><article><span className="stat-icon"><CalendarDays /></span><div><strong>{todayCount}</strong><small>Arrivals today</small></div></article><article><span className="stat-icon"><Stethoscope /></span><div><strong>{consultationCount}</strong><small>In consultation</small></div></article></div>
    ...
    <div className="admin-patient-workspace">
      ...
      <div className="result-heading"><div><strong>Clinic patients</strong><span>{filtered.length} {filtered.length === 1 ? 'record' : 'records'}</span></div>{(query || date || status !== 'All') && <button onClick={() => { setQuery(''); setDate(''); setStatus('All'); }}>Clear all filters</button>}</div>
      ...
      {loading ? <div className="admin-patient-empty"><div className="loading-ring"/><strong>Loading clinic patients…</strong></div> : <div className="admin-patient-list">{filtered.length ? filtered.map(v => {
        ...
        return <article className="admin-patient-card" key={v.id}>
          ...
          <div className="admin-card-actions"><button className="select-products-link" onClick={()=>choosePatient(v)}>Select Products</button><Link className="patient-details-link" to={`/admin/patients/${v.id}`}>Open patient <ChevronRight/></Link></div>
        </article>;
      }) : <div className="admin-patient-empty"><UserRoundCheck size={42}/><strong>No patients have been sent to the clinic</strong><p>{query || date || status !== 'All' ? 'Try adjusting your search or filters.' : 'Use Add Patient or wait for reception to send a patient to Admin.'}</p></div>}</div>}
    </div>
